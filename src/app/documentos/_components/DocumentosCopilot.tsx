...
export function DocumentosCopilot({
  ...
  return (
    <section className="hidden overflow-hidden rounded-3xl border border-slate-200 bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(15,23,42,0.92))] text-white shadow-2xl shadow-slate-900/10 xl:fixed xl:right-8 xl:top-6 xl:z-20 xl:block xl:w-[340px] xl:max-h-[calc(100vh-3rem)] xl:overflow-auto">
      ...
      <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            {SUGESTOES.map((sugestao) => (
              <button
                ...
                className="rounded-2xl border border-white/10 bg-white/5 px-3 py-3 text-left text-xs text-slate-200 transition hover:border-cyan-300/40 hover:bg-cyan-400/10 hover:text-white"
              >
                {sugestao}
              </button>
            ))}
            ...
            <div className="space-y-4 rounded-3xl border border-white/10 bg-white/5 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-100">{response.reply}</p>
                </div>
                <button
                  type="button"
                  onClick={() => onApplyFilters(response.filters)}
                  className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-400/10 px-3 py-2 text-xs font-semibold text-cyan-100 transition hover:bg-cyan-400/20"
                >
                  Aplicar filtros
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {response.results.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-left transition hover:border-cyan-300/30 hover:bg-slate-950/70"
                  >
                    ...
                    <div className="mt-3 space-y-1 text-xs text-slate-300">
                      <p>
                        Tipo:{" "}
                        <span className="text-slate-100">{item.tipo}</span>
                      </p>
                      <p>
                        Enviado em:{" "}
                ...
                {response.results.length === 0 && (
                  <div className="rounded-2xl border border-dashed border-white/15 px-4 py-6 text-sm text-slate-300 sm:col-span-2 xl:col-span-3">
        ...
        <div className="border-t border-white/10 bg-white/5 px-5 py-5 lg:border-l lg:border-t-0">
