...
export default function ContratosPage() {
  ...
  const carregarContratos = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Não foi possível carregar os contratos.",
      );
      setRegistros([]);
    } finally {
      setLoading(false);
  ...
  const baixarDocumento = async (registro: FormularioRecord) => {
    ...
    try {
      ...
      setError(null);
      const signedUrl = await getSignedFileUrl(path);
      const nome = getDocumentoNome(registro) || path.split("/").pop() || "contrato.pdf";
  ...
  const marcarComoRevisado = async (registro: FormularioRecord) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Não foi possível revisar o contrato.");
      }
      setRegistros((prev) =>
        prev.map((item) =>
          item.id === registro.id
      ...
      setError(err instanceof Error ? err.message : "Não foi possível revisar o contrato.");
    } finally {
      setReviewingId(null);
    }
  };
  ...
  const abrirEdicao = (registro: FormularioRecord) => {
    if (!canManageDocuments) return;
    const campos = getEditFields(TIPO_CONTRATOS);
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Não foi possível atualizar o contrato.");
      }
      setRegistros((prev) =>
        prev.map((item) =>
      ...
      setError(err instanceof Error ? err.message : "Não foi possível atualizar o contrato.");
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const confirmarRemocao = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Não foi possível remover o contrato.");
      }
  ...
  const editFields: EditField[] = editDialog ? getEditFields(TIPO_CONTRATOS) : [];
